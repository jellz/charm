...
export class EventManager {
	...
	registerEventHandler(handler: EventHandler) {
		...
		let handlerOverrode: EventHandler | undefined = this.eventStore.get(
			handler.id
		);
		if (handlerOverrode) {
	...
	deregisterHandler(handler: EventHandler) {
		if (!this.eventStore.has(handler.id))
			throw Error('You cannot deregister a non-existent handler');
